var createScene = function () {
    ...
    */
    ...
    uniform vec3  u2_MatDiffuse;   // Diffuse color (RGB)
    uniform vec3  u2_MatSpecular;  // Specular color (RGB)
    uniform float u2_MatShininess; // Phong exponent. higher = tighter specular highlight (e.g., 16, 32, 64)

    // Camera

    // Point light (emits from a position, with distance falloff)
    uniform vec3 u2_PointLightColor; // Linear RGB intensity/color of the point light
    uniform vec3 u2_PointLightPos;   // World-space position of the point light    
        ...
        vec3 lColor, vec3 lPos, vec3 N, vec3 V, vec3 kd, vec3 ks, float shin)
    {
        ...
        */
        ...
        R = reflect(-L,N);
        // YOUR CODE HERE
        RGB = kd*lColor*max(dot(N,L),0.0) + ks*lColor*pow(max(dot(R,V),0.0),shin) ;
         
        return RGB; // Dummy return value. Please change this
    }
    ...
    void main()
    {
        ...
        color += shadePointLight(
            ...
            u2_PointLightPos,
            N, V,
            u2_MatDiffuse, u2_MatSpecular, u2_MatShininess
        );
    ...
    // Material
    shaderMat.setFloat("uMatAmbient", 0.1)
    shaderMat.setColor3("uMatDiffuse",  new BABYLON.Color3(0.3, 0.7, 0.3));
    shaderMat.setColor3("uMatSpecular", new BABYLON.Color3(0.5, 1, 1));
    shaderMat.setFloat("uMatShininess", 45.0);

    // Point Light
    shaderMat.setColor3("uPointLightColor", new BABYLON.Color3(0.7, 0.4, 1.0));
    shaderMat.setVector3("uPointLightPos", new BABYLON.Vector3(7, 3, -1.0));




    shaderMat.setColor3("u2_MatDiffuse",  new BABYLON.Color3(0.9, 0.9, 0.5));
    shaderMat.setColor3("u2_MatSpecular", new BABYLON.Color3(0.9, 0.1, 1));
    shaderMat.setFloat("u2_MatShininess", 70.0);

    // Point Light
    shaderMat.setColor3("u2_PointLightColor", new BABYLON.Color3(0.9, 0.1, 0.8));
    shaderMat.setVector3("u2_PointLightPos", new BABYLON.Vector3(-5, 1, -2.0));
